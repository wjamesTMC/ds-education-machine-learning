...
# Smoothing is a very powerful technique used all across data analysis. Other
# names given to this technique are curve fitting and low band pass filtering.

# It's designed to detect trends in the presence of noisy data in cases in which
# the shape of the trend is unknown. The smoothing name comes from the fact that
...
# We assume that for any given day x, there's a true preference among the
# electorate, f(x), but due to the uncertainty introduced by polling, each data
# point comes with an error, epsilon.

# A mathematical model for the observed poll margin, y, is y equals f(x) + epsilon.
...
# The line we see does not appear to describe the trend very well. Note, for
# example, that on September 4, this is day negative 62, 62 days until Election
# Day, the Republican Convention was held. This appeared to give McCain a boost
# in the polls, which can be clearly seen in the data. The regression line does
# not capture this. To further see the lack of fit, we note that points above
# the fitted line, blue, and those below, red, are not evenly distributed. We
# therefore need an alternative, a more flexible approach.
