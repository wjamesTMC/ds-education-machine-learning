...
# cases with many predictors. Let’s go back to the digits example in which we
# had 784 predictors. For illustrative purposes, we will start by simplifying
# this problem to one with 2 predictors and two classes. Specifically, we define
# the challenge as building an algorithm that can determine if a digit is a 2 or
# 7 from the predictors. We are not quite ready to build algorithms with 784
...
# We need something more flexible. A method that permits estimates with shapes
# other than a plane.

# We are going to learn a few new algorithms based on different ideas and
# concepts. But what they all have in common is that they permit more flexible
# approaches. We will start by describing nearest neighbor and kernel
# approaches. To introduce the concepts behinds these approaches, we will again
# start with a simple one dimensional example and describe the concept of
# smoothing.
