...
# We describe how [? methods ?] lda
# and qda are not meant to be used with datasets that have many predictors.
# This is because the number of parameters that we need to estimate
# becomes too large.
# For example, with the digits example where we have 784 predictors,
# lda would have to estimate over 600,000 parameters.
# With qda, you would have to multiply that
# by the number of classes, which is 10 here.
# Kernel methods such k-nearest neighbors or local regression
# do not have model parameters to estimate.
...
# A useful way to understand the curse of dimensionality
# is by considering how large we have to make
# a neighborhood, the neighborhood we used to make the estimates,
# to include a given percentage of the data.
...
# spaced points in the [? 01 ?] interval, and you wanted to create windows that
# include 1/10 of the data.
# Then it's easy to see that our windows have to be of size 0.1.
# You can see it in this figure.
# Now, for two predictors, if we decide to keep the neighborhood just a small, 10%
# of each dimensions we only include one point.
# If we want to include 10% of the data, then we
...
# This proportion gets close to 1, which means
# we're including practically all the data,
...
# In this video, we introduced a set of elegant and versatile methods
# that adapt to higher dimensions and also allow
# these regions to take more complex shapes, while still
# producing models that are interpretable.
# These are very popular well-known and studied methods.
# We will focus on regression and decision trees
